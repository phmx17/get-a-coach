<template>
  <form @submit.prevent="submitForm">
    <div class="form-control" :class="{invalid: !firstName.isValid}">  <!-- validation css  -->
      <label for="firstname">Firstname</label>
      <input type="text" id="firstname" v-model.trim="firstName.val" @blur="clearValidity('firstName')" />
      <p v-if="!firstName.isValid">Firstname must not be empty.</p>  <!-- validation message -->
    </div>
...
export default {
  ...
  methods: {
    clearValidity(inputField) {
      this[inputField].isValid = true;  // [] = computed value; ALSO no '.' after 'this' !
    },
    validateForm() {
    ...
    submitForm() {
      this.validateForm();  // call method above
      if (!this.formIsValid) {
        return;
      }
     const formData = {
